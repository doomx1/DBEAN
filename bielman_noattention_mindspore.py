...
# 定义双向Elman RNN模型（无注意力层）
class BiElmanNoAttentionNet(nn.Cell):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_classes, num_layers=2, pad_idx=0):
        super(BiElmanNoAttentionNet, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim,
                                      embedding_table=embedding_matrix_ms,
                                      padding_idx=pad_idx)
    ...
    def construct(self, x):
        ...
        rnn_out, _ = self.rnn(emb)            # (batch, seq_len, 2*hidden_dim)
        norm_out = self.layernorm(rnn_out)    # layer norm
        # 无注意力，做均值池化
        context = ops.ReduceMean()(norm_out, 1)  # (batch, 2*hidden_dim)
...
num_layers = 2
num_classes = 4
model = BiElmanNoAttentionNet(vocab_size, embed_dim, hidden_dim, num_classes, num_layers, pad_idx=pad_idx)
...
def train_one_epoch(model, dataset, epoch):
    ...
    for data_item in tqdm(dataset.create_tuple_iterator(), desc=f"Epoch {epoch} Training"):
        texts, labels = data_item
        def forward_fn(x, y):
            logits = model(x)
            return loss_fn(logits, y)

        loss = forward_fn(texts, labels)
        grads = grad_fn(forward_fn, model.trainable_params())(texts, labels)
        optimizer(grads)
        ...
        preds = np.argmax(model(texts).asnumpy(), axis=1)
    ...
    save_checkpoint(model, f"checkpoints/mindspore_model_weights_{epoch}_noattention.ckpt")
...
def evaluate(model, dataset):
    ...
    for data_item in tqdm(dataset.create_tuple_iterator(), desc="Evaluating"):
        texts, labels = data_item
        logits = model(texts)
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Loss Curve (No Attention)")
plt.legend()
plt.tight_layout()
plt.savefig("ms_loss_curve_no_attention.png")
plt.close()
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("Accuracy Curve (No Attention)")
plt.legend()
plt.tight_layout()
plt.savefig("ms_accuracy_curve_no_attention.png")
plt.close()

cm = confusion_matrix(test_labels_eval, test_preds)
plt.figure()
plt.imshow(cm, cmap=plt.cm.Blues)
plt.title("Confusion Matrix (No Attention)")
plt.colorbar()
...
plt.tight_layout()
plt.savefig("ms_confusion_matrix_no_attention.png")
plt.close()

print("Training and evaluation completed (MindSpore, no attention).")
